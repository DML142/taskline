...
Before beginning work, create a focused plan in `docs/superpowers/plans/` with a checklist of phases and steps. Update that plan before and after every phase, marking completed work with `[x]`, outstanding work with `[ ]`, and any intentional deviation from the plan.
...
Use clear, human commit messages and pull requests without AI-related wording. Keep pull-request descriptions brief and concrete: short bullets such as `added routes: GET /…` or `comments on the project route` are preferred over exhaustive implementation narration.
